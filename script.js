...
if (/umpir(?:e|ing)?|lbw|drs/.test(t)) return 'Umpiring';
return 'Any';
}
...
// Only return the filtered questions; fall back to all ONLY if zero matched
let items = sub === 'Any' ? [...bank.cricket] : bank.cricket.filter(q => q.tag === sub);
if (items.length === 0) items = [...bank.cricket];
return items.map(q => ({
text: q.template(),
sample: q.sample(),
...
const topic = $("topic").value.trim(); // e.g., "batting" or "umpiring"
