...
function learn(meta, q) {
  ...
  if (!store[key].some(e => norm(e.text) === norm(rec.text))) {
    ...
    store[key] = store[key].slice(0, 400); // keep most recent 400
    saveLearned(store);
  }
}
...
let state = {
  ...
  seen: {},          // { comboKey: Set(normalizedProblem) }
  mcqCount: 0,
  openCount: 0
};
...
function sanitizePayload(payload) {
  ...
  if (!hasMCQ) throw new Error('MCQ required but missing choices.');
...
/* ------------------------------- prompts ---------------------------------- */
async function aiQuestionMessages(meta, dedupHints = [], indexInBatch = 0, attempt = 0, banNumbers = []) {
  ...
  const learned = getLearned(meta).slice(0, 6).map(r => r.text).filter(Boolean);
  const avoidTexts = [...learned, ...dedupHints].slice(-10).join("\n---\n");
...
- Include exactly 4 plausible options with ONE correct (similar magnitude; no giveaways).
- The problem MUST be NEW for this (area, level, grade, sport) combo and DIFFER from recent items.
- Produce JSON only.
Well-posedness rules:
- If using “each thing is worth K” and asking “how many things”, pick numbers so the division is an INTEGER.
- Do NOT use ambiguous “same number each game/round” unless you also provide enough info to determine a unique answer.
- Avoid underspecified sports scoring (e.g., mixed touchdowns/field-goals) unless totals align with the asked quantity.
Tone: Friendly, concise classroom coach.`;
  ...
  const userPayload = {
    REQUIREMENTS: {
      ...
      PAIR_INDEX: indexInBatch, TYPE: "MCQ-ONLY"
    },
...
async function aiSampleMessages(meta, plainProblem, forcedAnswer /* optional */) {
  const area = metaToArea(meta);
  const level = meta.level || 'easy';
  const grade = meta.grade || '8';

  const mustLine = (typeof forcedAnswer === 'number' && isFinite(forcedAnswer))
    ? `\n- FINAL NUMERIC ANSWER MUST EQUAL: ${forcedAnswer}\n- Ensure steps and computation match this value exactly.`
    : '';
...
/* ------------------- checker + numeric/MCQ verification ------------------- */
...
function extractFinalAnswerNumber(html) {
  if (!html) return NaN;
  const clean = html.replace(/<(?!br\s*\/?>|strong\b)[^>]*>/gi, " ");
  const m = clean.match(/answer\s*:\s*([^<\n]+)/i);
  return m ? parseNumberish(m[1]) : NaN;
}

// simple G4–6 heuristic: (sum) / unit value
function heuristicAnswer(problemText) {
...
async function aiCheckMessages(meta, plainProblem) {
  return [
    {
      role: "system",
      content:
        "You are Math Trainer LLM Checker. Compute the single numeric final answer ONLY.\n" +
        "Return STRICT JSON: {\"answer\": <number or null>} with no extra text.\n" +
        "If info is insufficient, use null. Do NOT explain."
    },
    {
      role: "user",
      content: `Problem:\n${plainProblem}\n\nReturn ONLY: {"answer": number|null}`
    }
  ];
}
async function llmCheckAnswer(meta, plainProblem) {
  try {
    const msgs = await aiCheckMessages(meta, plainProblem);
    const raw = await llmAsk(msgs, { max_tokens: 60 });
    const parsed = jsonExtract(raw);
    const a = parsed && (typeof parsed.answer === 'number' ? parsed.answer : null);
    return (a === null || !isFinite(a)) ? null : a;
  } catch { return null; }
}

async function enforceMcqCorrectness(q, meta) {
  if (!q || !q.choices || q.choices.length !== 4) return;

  // 1) Get a verified numeric answer (heuristic → checker)
  let verified = heuristicAnswer(q.plainProblem);
  if (verified === null) verified = await llmCheckAnswer(meta, q.plainProblem);
  ...
  let idx = q.choices.findIndex(c => {
    ...
    return isFinite(n) && equalish(n, verified);
  });

  if (idx >= 0) {
    q.answer_index = idx;
    q.correct = q.choices[idx];
    return;
  }
  ...
  // ensure uniqueness among decoys
  for (let i = 0; i < q.choices.length; i++) {
    if (i === replaceAt) continue;
    if (q.choices[i].trim() === nice) {
      const n = parseNumberish(q.choices[i]);
      q.choices[i] = isFinite(n) ? String(n + 1) : q.choices[i] + " ";
    }
  }
  q.answer_index = replaceAt;
  q.correct = nice;
}
...
function normalizeConcise(html) {
  ...
  const starters = [
    /^to\s+find[^:.,]*[:.,-]\s*/i,
    /^we\s+need\s+to\s*/i,
    /^(first|then|next|therefore|so|thus),?\s*/i
  ];
  ...
  const capLine = s => (s.length > maxLineLen ? s.slice(0, maxLineLen - 1) + "…" : s);
  const answer = lines.find(l => /<strong>Answer:/i.test(l)) ?? lines[lines.length - 1];
  const head = lines.filter(l => l !== answer).slice(0, 2).map(capLine);
  lines = [...head, answer];
...
/* ------------------------ validation (well-posedness) --------------------- */
function validateQuestion(q) {
  const t = String(q.plainProblem || q.text || '').toLowerCase();

  // Reject ambiguous “same number each game/round” unless total is explicitly given
  if (/same number/.test(t) && /each game|each half|each round/.test(t)) {
    if (!/\b(total|together)\b.*\b(field goals?|touchdowns?|baskets?|goals?)\b/.test(t)) {
      return { ok: false, reason: 'ambiguous_same_number' };
    }
  }

  // If asks for count with “each … worth K”, ensure at least one total is divisible by K
  const worthK = t.match(/\beach\b[^.]*?\bworth\b[^.\d]*(\d+)/i);
  if (worthK) {
    const k = parseFloat(worthK[1]);
    const nums = (t.match(/\d+/g) || []).map(Number);
    const totals = nums.filter(n => n !== k);
    const anyDiv = totals.some(n => k !== 0 && n % k === 0);
    if (!anyDiv) return { ok: false, reason: 'non_integer_division' };
  }

  // Basic MCQ structure
  if (!q.choices || q.choices.length !== 4 || !Number.isInteger(q.answer_index)) {
    return { ok: false, reason: 'mcq_shape' };
  }
  return { ok: true };
}

/* --------------------------- builders (MCQ-only) -------------------------- */
async function aiBuildQuestion(meta, dedupHints = [], indexInBatch = 0, attempt = 0, banNumbers = []) {
  const msgs = await aiQuestionMessages(meta, dedupHints, indexInBatch, attempt, banNumbers);
  ...
  // Correct MCQ if LLM slipped on answer_index or options
  await enforceMcqCorrectness(q, meta);

  learn(meta, q);
  return q;
}

/* ---------- ensure NEW, VALID question each time with retries ------------- */
async function generateUniqueQuestion(meta, indexInBatch = 0) {
  ...
  for (let attempt = 0; attempt < 8; attempt++) {
    const q = await aiBuildQuestion(meta, recentTexts, indexInBatch, attempt, bannedNumbers);
    const v = validateQuestion(q);
    const text = norm(q.plainProblem);

    if (v.ok && text && !seenSet.has(text)) {
      seenSet.add(text);
      return q;
    }
  ...
  // last resort: return last built, but it's been corrected for MCQ answer already
  const last = await aiBuildQuestion(meta, recentTexts, indexInBatch, 6, bannedNumbers);
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  $("btn-sample")?.addEventListener('click', async () => {
    ...
    setBusy(true);
    try {
      // 1) initial concise solution
      let msgs = await aiSampleMessages(state.meta, q.plainProblem || $("problem").innerText);
      let solution = await llmAsk(msgs, { max_tokens: 180 });
      solution = normalizeConcise(solution);
      solution = shortenSolution(solution);

      // 2) verify numeric final answer
      let verified = heuristicAnswer(q.plainProblem);
      if (verified === null) verified = await llmCheckAnswer(state.meta, q.plainProblem);
      ...
      if (verified !== null && isFinite(verified)) {
        ...
        if (!isFinite(shown) || !equal(shown, verified)) {
          ...
          msgs = await aiSampleMessages(state.meta, q.plainProblem, verified);
          let fixed = await llmAsk(msgs, { max_tokens: 180 });
          fixed = normalizeConcise(fixed);
          fixed = shortenSolution(fixed);

          // patch last line if STILL wrong
          if (!equal(extractFinalAnswerNumber(fixed), verified)) {
            const parts = fixed.split(/<br\s*\/?>/i).map(s => s.trim()).filter(Boolean);
            const head = [parts[0] || '', parts[1] || ''];
            const finalNum = Number.isInteger(verified) ? String(verified) : String(verified);
            fixed = `${head[0]}<br>${head[1]}<br><strong>Answer: ${finalNum}</strong>`;
          }
          solution = fixed;
        }
        $("status").innerHTML = `<em>Solution (✓ Verified):</em><br>${solution}`;
      } else {
        $("status").innerHTML = `<em>Solution (unverified):</em><br>${solution}`;
      }
